(function () {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            $('#tablaasingas tbody').on('click', '.btn-editar', function () {
                const dataMatri2 = table.row($(this).parents('tr')).data();
                console.log(dataMatri2);

                // Llenar los campos del formulario FormAnioGradoTurno
                $('#Id_asignadasasig').val(dataMatri2.Id);
                $('#Grado2').val(dataMatri2.Id_grado);
                $('#Turno2').val(dataMatri2.Id_turno);
                $('#AnioLectivo2').val(dataMatri2.Id_anio_lectivo);
            });
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            response.forEach(function (grado) {
                ...
                $('#Grado2').append('<option value="' + grado.Id + '">' + grado.Nombre + '</option>');
            });
        },
        error: function (xhr, status, error) {
            // Manejar errores de la solicitud AJAX
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            response.forEach(function (Turnos) {
                ...
                $('#Turno2').append('<option value="' + Turnos.Id + '">' + Turnos.Nombre + '</option>');
            });
        },
        error: function (xhr, status, error) {
            // Manejar errores de la solicitud AJAX
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            response.forEach(function (anioLec) {
                ...
                $('#AnioLectivo2').append('<option value="' + anioLec.Id + '">' + anioLec.anio + '</option>');
            });
        },
        error: function (xhr, status, error) {
            // Manejar errores de la solicitud AJAX
    ...
    $(document).ready(() => {
        ...
        $("#GuardarFormAnioGradoTurnoEdit").click(async function () {

            let FormAnioGradoTurnoEdit = new FormData($("#FormAnioGradoTurnoEdit")[0]);
            FormAnioGradoTurnoEdit= formDataToObject(FormAnioGradoTurnoEdit);

    
            $.ajax({
                url: '/api/grado/calificacion/edit',
                method: 'POST',
                dataType: 'json',
                data: FormAnioGradoTurnoEdit,
                success: async function (response) {
                    console.log(response);    
                    if (response.exito) {
                        $('#FormAnioGradoTurnoEdit')[0].reset();      
                        await cargarLista();
